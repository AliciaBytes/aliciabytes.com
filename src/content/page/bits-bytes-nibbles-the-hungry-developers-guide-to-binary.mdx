...
import Information from '@src/components/Information.astro';

<Information text="This blog post is currently in a draft state and still needs to be edited and adjusted. I'm already putting it out because I used to be a perfectionist and never release anything because it was never perfect enough and I'm trying to put more things out. I'll revisit and refine content along the way." />

If you want to know how computers work under the hood and what's going on how the data is made up inside the memory you won't get around bits, bytes an binary. In this post I want to introduce you to and dive into how all of this works. We look into it from the first starts and explain everything. Including how tech people are dorks with naming things, and apparently hunger. Just look at some of the [obsolete units](https://en.wikipedia.org/wiki/Units_of_information#Obsolete_and_unusual_units) that people used for it. It has such gems as:
...
The digit in the 0th position at the very right is easy since it's always the value that's written there times 1, no matter what number system we're using, since any number to the power of 0 always results into 1. But for the other positions we'll have to redo it. So let's try the same example with a binary number. In binary we're calculating with base 2 instead of base 10 numbers, so we need to take the powers of 2 instead of powers of 10 for the calculations.
...
With that said, there will still be plenty of times where those topics will come up again and you'll learn more about them, but for now it's really enough (this blog already got quite long 😅).

I'm planning to write more blog posts about IT and Computer Science from the ground up and turning them into a series, so feel free and follow along if you're interested in more.
